...
export default function RelationshipExplorer({ data }: { data: ExplorerData }) {
  ...
  return <main className="shell">
    ...
    {data.declared && <details className="evidence-drawer declared-catalog" open={Boolean(data.declared.issue)}>
      ...
      {data.declared.shopApp && <div className="declared-joins"><h3>{data.declared.shopApp.label}</h3><p>Private repo default branch <code>{data.declared.shopApp.defaultBranch}</code> · commit <code>{data.declared.shopApp.verifiedCommit.slice(0, 7)}</code> · runtime <code>{data.declared.shopApp.runtimeDataSource}</code>. Vehicle key <code>{data.declared.shopApp.vehicleKey}</code>. Ninox table IDs in the shop app: {data.declared.shopApp.ninoxTableIds.length ? data.declared.shopApp.ninoxTableIds.join(", ") : "none found"}.</p>{data.declared.shopApp.yardPhases.length > 0 && <p>Yard / Work Plan phases: {data.declared.shopApp.yardPhases.join(", ")}. These are not Ninox mechanic_status (E.TA) values.</p>}</div>}
    </details>}
