...
import type { StructuralDiff } from "../src/scanner/scanHistory.js";
...
export async function loadExplorerData(): Promise<ExplorerData> {
  const [schema, relationshipArtifact, summary, shopMap, quality, history] = await Promise.all([
    ...
    readOptionalJson<StructuralDiff>("output/history/latest-diff.json"),
  ]);
  ...
  return {
    ...
    quality,
    history: summarizeStructuralDiff(history),
  };
}
